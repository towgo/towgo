...
type Dialect interface {
	...
	Alias(string) string       // return what a sql types's alias of
	ColumnTypeKind(string) int // database column types kind
	...
	DropColumnSQL(tableName string, colName string) string
...
func (db *Base) DropColumnSQL(tableName string, colName string) string {
	quoter := db.dialect.Quoter()
	sql := fmt.Sprintf("ALTER TABLE %v DROP COLUMN %v", quoter.Quote(tableName), quoter.Quote(colName))
	return sql
}
...
func regDrvsNDialects() bool {
	providedDrvsNDialects := map[string]struct {
		...
		"oracle":   {"oracle", func() Driver { return &godrorDriver{} }, func() Dialect { return &oracle{} }},
	}
