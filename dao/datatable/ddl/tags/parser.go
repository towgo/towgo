...
// ErrUnsupportedType represents an unsupported types error
...
// Parser represents a parser for xorm.io tag
type Parser struct {
	identifier   string
...
func (parser *Parser) ParseWithCache(v *models.StrutTable) (*schemas.Table, error) {
	t := v.TableName
...
// ClearCacheTable removes the database mapper of a types from the cache
func (parser *Parser) ClearCacheTable(t string) {
...
func (parser *Parser) Parse(v *models.StrutTable) (*schemas.Table, error) {

	table := schemas.NewEmptyTable()
	table.Type = reflect.TypeOf(v)
	table.Name = v.TableName
	table.Comment = v.Comment

	for i := 0; i < len(v.Fields); i++ {
		field := v.Fields[i]
		col, err := parser.parseField(table, i, field, reflect.ValueOf(field))
