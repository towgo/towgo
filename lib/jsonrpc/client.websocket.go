	...
	go func(ctx context.Context, rpcConn *WebScoketClient) {
		for {
			...
			default:
				...
				if wsc.PingDuringTime == 0 {
					wsc.PingDuringTime = 5
				}
				time.Sleep(time.Second * time.Duration(wsc.PingDuringTime))
				if !hasResponse {
...
func NewWebsocketClient(url, origin string) *WebScoketClient {
	wsc := &WebScoketClient{
		...
		origin:        origin,
		autoReConnect: true,
		UserAgent:     "towgo websocket client",
	}
	return wsc
}
...
func (wsc *WebScoketClient) ReLoad(url, origin string) {
	...
	request.Header.Set("User-Agent", wsc.UserAgent)

	// Dial with custom request
