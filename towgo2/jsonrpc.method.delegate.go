...
type (
	JsonRpcFunc   func(rpcConn JsonRpcConnection)
	JsonRpcServer struct {
		funcs         map[string]*Api
		lock          sync.Mutex
		lockedMethods sync.Map
		beforExec     JsonRpcFunc
		afterExec     JsonRpcFunc
		defaultExec   JsonRpcFunc
	}
)
...
func (j *JsonRpcServer) SetDefaultFunc(f JsonRpcFunc) {
	j.defaultExec = f
}
func (j *JsonRpcServer) SetBeforeExec(f JsonRpcFunc) {
	j.beforExec = f
}
func (j *JsonRpcServer) SetAfterExec(f JsonRpcFunc) {
	j.afterExec = f
}

var BeforExec JsonRpcFunc
var AfterExec JsonRpcFunc
var DefaultExec JsonRpcFunc = func(rpcConn JsonRpcConnection) {
